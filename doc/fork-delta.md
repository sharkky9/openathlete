...
**Upgrade test:** `pnpm web test` (`auth-provider.test.tsx`). By hand: sign in, log out through the
sidebar user menu (not a reload), sign up a new account, and confirm it lands on onboarding.
...
**Upgrade test:** `pnpm web test` (`messages-page.test.tsx`, `chat-window.test.tsx`,
`new-message-thread-dialog.test.tsx`) and `pnpm api test`
(`message-thread.service.spec.ts`). By hand: on an account with no coach or athlete link, open
Messages and the chatbot bubble and confirm the network tab shows no `POST /messages/threads`.
...
**Upgrade test:** `pnpm check:locale-parity` and `pnpm web test`
(`new-message-thread-dialog.test.tsx`), then open the dialog with the language switcher on English.
...
**Upgrade test:** `pnpm api test` (`user-roles.spec.ts`) and `pnpm web test`
(`roles-section.test.tsx`). By hand: onboard as athlete only, then add the coach role from
Settings -> Profile and confirm the Athletes tab appears without a reload.

## Web unit tests (Vitest)

**Reason:** `apps/web` had no test runner at all, so every bug fixed in the web app — #34, #36, #37,
#39 — could only be verified by hand in a browser.

**Implementation:** `vitest`, `jsdom` and Testing Library in `apps/web` devDependencies, with
`vitest.config.ts` (jsdom, Paraglide plugin so the gitignored `src/paraglide` output exists),
`vitest.setup.ts` (Radix needs `ResizeObserver`, `scrollIntoView` and `matchMedia`, none of which
jsdom implements) and `test` / `test:watch` scripts. `.github/workflows/tests.yml` gains a `web-unit`
job, `Web unit tests (Vitest)`, alongside the existing `api-unit`; like the `checks.yml` jobs it is
not a required check on `main` yet, which is a ruleset change rather than a change in this tree. It
runs no Paraglide compile step, unlike `lint.yml` and `type-check.yml`, because `vitest.config.ts`
loads `paraglideVitePlugin` itself and generates `src/paraglide` as part of the run.

`vitest.config.ts` is deliberately separate from `vite.config.ts`: the app build loads Tailwind and
derives `base` from the Capacitor env var, neither of which a jsdom test needs.

**Upstream modifications:** `apps/web/package.json` (scripts and devDependencies), `pnpm-lock.yaml`.
`.github/workflows/tests.yml` is fork-owned, so the new job cannot conflict.

**Upstream candidate:** yes.

**Removal condition:** upstream adds its own web test runner; then take upstream's.

**Upgrade test:** `pnpm web test`.
...
**Upstream modifications:** `apps/api/package.json` (the Jest `setupFiles` entry only — the
`moduleNameMapper` and ts-jest `tsconfig` lines in the same block belong to "Unauthenticated API
surface hardened" above), plus the added `apps/api/jest-setup.ts`.
